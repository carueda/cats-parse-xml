...
def parseAllUnderDirectory(directory: os.Path, showAst: Boolean): Unit =
  ...
  failures.foreach { case (file, _) => println(s"  $file") }
...
val usage =
  s"""
    |Usage: main.sc --file <filename> [ --showAst ]
    |       main.sc --dir <directory> [ --showAst ]
    |""".stripMargin
...
if opts.wereGiven then
  opts.file foreach { filename =>
    parseOne(path(filename), opts.showAst)
  }
  opts.dir foreach { dir =>
    parseAllUnderDirectory(path(dir), opts.showAst)
  }
else println(usage)
